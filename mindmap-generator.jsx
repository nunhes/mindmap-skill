...
const MindMapGenerator = () => {
  ...
  const [nodesToMove, setNodesToMove] = useState([]); // Changed from ref to state
  ...
  useEffect(() => {
    ...
    fetch('./claude-skills-mindmap.opml')
      .then(response => response.text())
      .then(opmlText => {
        const parser = new DOMParser();
        ...
        const radius = 20; // Distance from center to first level
        ...
        const processNode = (outline, parentId, level, parentAngle, angleRange, nodeColor) => {
          ...
          if (level === 1) {
            ...
            const distance = 15; // Distance from parent
          ...
          if (hasChildren) {
            ...
            const effectiveRange = angleRange * spreadMultiplier;
  ...
  const handleMouseDown = (e, nodeId) => {
    ...
    setNodesToMove(allNodesToMove); // Use state instead of ref
  ...
  const handleMouseUp = () => {
    ...
    setNodesToMove([]); // Clear the nodes to move
    setIsPanning(false);
  };
  ...
  return (
    <div
      className="w-full h-screen bg-emerald-50 overflow-hidden"
          ...
          >
            <svg className="absolute inset-0 w-full h-full" style={{ zIndex: 0 }}>
              {Object.values(nodes).map((node) => {
                ...
                const isBeingDragged = draggingNode && nodesToMove.includes(node.id);
                const parentBeingDragged = draggingNode && nodesToMove.includes(node.parentId);
            ...
            {Object.values(nodes).filter(isNodeVisible).map((node) => {
              ...
              return (
                <div
                  key={node.id}
                  className={`absolute transform -translate-x-1/2 -translate-y-1/2 cursor-move ${draggingNode === node.id ? '' : 'transition-all duration-200'}`}
                  style={{
                    left: `${x}%`,
                    top: `${y}%`,
                ...
                >
                  <div className="relative">
                    <div
                      className={`
                        ...
                        ${selectedNode === node.id ? 'scale-115 ring-4 ring-offset-2' : ''}
                      ...
                      style={{
                        ...
                        ...(selectedNode === node.id && { '--tw-ring-color': node.color })
                      }}
                    >
                      {node.label}
                    </div>
                    ...
                    {node.hasChildren && hoveredNode === node.id && (
                      <>
                        <div
                          className="absolute -right-3 -top-3 w-6 h-6 rounded-full flex items-center justify-center cursor-pointer shadow-lg"
                          style={{
                        ...
                        {expandedNodes[node.id] && hasHiddenChildren(node.id) && (
                          <div
                            className="absolute -left-3 -top-3 w-6 h-6 rounded-full flex items-center justify-center cursor-pointer shadow-lg"
                            style={{
                              backgroundColor: node.color,
                              color: node.id === 'center' ? '#ffffff' : (isLightColor(node.color) ? '#000000' : '#ffffff'),
                              border: '3px solid white'
                  ...
                  {selectedNode === node.id && (
                    <div
                      className={`
                        absolute w-80 bg-white rounded-lg shadow-xl p-6 border border-gray-200 z-30
                        ${isBottomHalf ? 'bottom-full mb-4' : 'top-full mt-4'}
                        ${isRightHalf ? 'right-0' : 'left-0'}
                      `}
                    >
                      <h3 className="font-bold text-lg mb-3" style={{ color: node.color }}>{node.label}</h3>
                      {node.description && (
                        <p className="text-gray-700 text-sm mb-4 leading-relaxed">{node.description}</p>
                      )}

                      {node.children && (
                        <div className="space-y-3 pt-3" style={{ borderTop: `2px solid ${node.color}` }}>
                          {node.children.map((child, i) => (
                            <div key={i} className="flex flex-col">
                              <span className="font-semibold text-sm" style={{ color: node.color }}>{child.label}</span>
                              <span className="text-gray-600 text-xs mt-1">{child.detail}</span>
                            </div>
                          ))}
                        </div>
                      )}
